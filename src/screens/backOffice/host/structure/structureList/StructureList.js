...
const StructureList = (props) => {
  ...
  const disableStructure = (id) => async () => {
    // const HEADER = decryptItem(props.tokenDuck.token);
    const HEADER = getLocalStorage("token");
    let disableStructureApi = await disableStrutturaPutApi(
      id,
      { esito: true },
      HEADER
    );
  ...
  const goToAddEditStructure =
    ...
    () => {
      navigate(
        `/${routes.DASHBOARD}/${routes.STRUCTURE_OPERATION}/${
          idStructure === null ? "new" : idStructure
        }`,
        {
    ...
    return (
      <HorizontalCard
        ...
        text={
          <>
            <span>
              {!structure.indirizzo ? "Some city" : structure.indirizzo.citta},{" "}
            </span>
            <span>
              {!structure.indirizzo
                ? "Some country"
                : structure.indirizzo.stato}
            </span>
            <p>
              {!structure.tipologiaStrutturaId
                ? "Structure"
                : structure.tipologiaStrutturaId.tipo}
            </p>
          </>
        }
        footerContent={
          <div className="rate_container">
            <div>
              <Rate
                className="rate_item"
                disabled
                defaultValue={structure.media_recensioni}
              />
          ...
          <>
            ...
            >
              <h1>
                ...
                {t("bo.screens.host.reservationList.confirmReservationDelete")}
              </h1>
              <p>
                Sei sicuro di voler disativare la struture, una volta disativata
                non sara visibile ai clienti di BeijeBnb
              </p>
            </Modal>
            <FontAwesomeIcon
              onClick={goToAddEditStructure(structure.id)}
              className="icon_edit"
              icon={faPen}
            />
            <FontAwesomeIcon
              onClick={disableStructure(structure.id)}
              className="icon_disable"
              icon={faTrashCan}
            />
          </>
        }
  ...
  const switchToPage = async (clickedPage) => {
    ...
    // const HEADER = decryptItem(props.tokenDuck.token);
    const HEADER = getLocalStorage("token");
    let dataPaginationStructure = await showHostStruttureGetApi(
      clickedPage,
      3,
      HEADER
    );
  ...
  return (
    ...
    >
      {!state.loading ? (
        state.structureListData.map(getCardStructures)
      ) : (
        <Spin />
      )}
    </CardList>
